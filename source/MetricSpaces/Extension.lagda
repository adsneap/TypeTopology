   ...
   where
    ...
    I ((u , v) , u<x , x<v , l₁ , l₂) = ∣ f u - 1ℚ - 1ℚ , ∣ (u , v , 1ℚ , 0 , refl) , u<x , x<v , IV , V ∣ ∣
     where
      ...
      II = f u - 1ℚ                   ＝⟨ ℚ-zero-right-neutral (f u - 1ℚ) ⁻¹ ⟩
           f u - 1ℚ + 0ℚ              ＝⟨ ap (f u - 1ℚ +_) (ℚ-inverse-sum-to-zero' 1ℚ ⁻¹) ⟩
           f u - 1ℚ + ((- 1ℚ) + 1ℚ)   ＝⟨ ℚ+-assoc (f u - 1ℚ) (- 1ℚ) 1ℚ ⁻¹ ⟩
           f u - 1ℚ - 1ℚ + 1ℚ         ∎
      III : abs (v - u) < δ'
      III = pos-abs-no-increase (v - u) δ' (l₁ , l₂)
      IV : abs (u - v) < δ'
      IV = transport (_< δ') (ℚ-metric-commutes v u) III
      V : f u - 1ℚ - 1ℚ + 1ℚ < f u
      V = transport (_< f u) II (order1ℚ' (f u))
   ...
   where
    ...
    I ((u , v) , u<x , x<v , l₁ , l₂) = ∣ (f v + 1ℚ + 1ℚ) , ∣ (u , v , 1ℚ , 0 , refl) , u<x , x<v , IV , V ∣ ∣
     where
      ...
      II = f v + 1ℚ              ＝⟨ ℚ-zero-right-neutral (f v + 1ℚ) ⁻¹ ⟩
           f v + 1ℚ + 0ℚ         ＝⟨ ap (f v + 1ℚ +_) (ℚ-inverse-sum-to-zero 1ℚ ⁻¹) ⟩
           f v + 1ℚ + (1ℚ - 1ℚ)  ＝⟨ ℚ+-assoc (f v + 1ℚ) 1ℚ (- 1ℚ) ⁻¹ ⟩
           f v + 1ℚ + 1ℚ - 1ℚ    ∎
      III : abs (v - u) < δ'
      III = pos-abs-no-increase (v - u) δ' (l₁ , l₂)
      IV : abs (u - v) < δ'
      IV = transport (_< δ') (ℚ-metric-commutes v u) III
      V : f v < f v + 1ℚ + 1ℚ - 1ℚ
      V = transport (f v <_) II (order1ℚ (f v))
    ...
    ltr = ∥∥-functor I
     where
      ...
      I : Σ (u , v , (ε , 0<ε)) ꞉ ℚ × ℚ × ℚ₊ , (u < x) × (x < v) × B-ℚ u v (pr₁ (uniform-modulus ℚ-metric-space ℚ-metric-space f is-continuous (ε , 0<ε))) (pr₂ (uniform-modulus ℚ-metric-space ℚ-metric-space f is-continuous (ε , 0<ε))) × p + ε < f u
        → Σ p' ꞉ ℚ , p < p' × p' ∈ L
      I ((u , v , (ε , 0<ε)) , u<x , x<v , u-v<δ , l) = II (ℚ-dense (p + ε) (f u) l)
       where
        δ : ℚ
  ...
  is-located p q p<q = ∥∥-functor
                        I
                         (ℝ-arithmetically-located x
                           (pr₁ (uniform-modulus ℚ-metric-space ℚ-metric-space f is-continuous (1/4 * (q - p) , ℚ<-pos-multiplication-preserves-order 1/4 (q - p) (0 , refl) (ℚ<-difference-positive p q p<q))))
                            (pr₂ (uniform-modulus ℚ-metric-space ℚ-metric-space f is-continuous (1/4 * (q - p) , ℚ<-pos-multiplication-preserves-order 1/4 (q - p) (0 , refl) (ℚ<-difference-positive p q p<q)))))
   where
    0<1/4q-p : 0ℚ < 1/4 * (q - p)
    0<1/4q-p = ℚ<-pos-multiplication-preserves-order 1/4 (q - p) (0 , refl) (ℚ<-difference-positive p q p<q)
    I : Σ (u , v) ꞉ ℚ × ℚ , (u < x) × (x < v) × (0ℚ < v - u) × (v - u < pr₁ (uniform-modulus ℚ-metric-space ℚ-metric-space f is-continuous (1/4 * (q - p) , ℚ<-pos-multiplication-preserves-order 1/4 (q - p) (0 , refl) (ℚ<-difference-positive p q p<q)))) → p ∈ L ∔ q ∈ R
    I ((u , v) , u<x , x<v , 0<v-u , v-u<δ) = Cases II III IV
     where
      ...
      ii = f v - f u + (1/4 * (q - p) + 1/4 * (q - p))         ＝⟨ ℚ+-assoc (f v - f u) (1/4 * (q - p)) (1/4 * (q - p)) ⁻¹ ⟩
            f v - f u + 1/4 * (q - p) + 1/4 * (q - p)           ＝⟨ ap (_+ 1/4 * (q - p)) (ℚ+-assoc (f v) (- f u) (1/4 * (q - p))) ⟩
            f v + ((- f u) + 1/4 * (q - p)) + 1/4 * (q - p)     ＝⟨ ℚ+-assoc (f v) ((- f u) + (1/4 * (q - p))) (1/4 * (q - p)) ⟩
            f v + ((- f u) + 1/4 * (q - p) + 1/4 * (q - p))     ＝⟨ ap (f v +_) (ℚ+-comm ((- f u) + (1/4 * (q - p))) (1/4 * (q - p))) ⟩
            f v + (1/4 * (q - p) + ((- f u) + 1/4 * (q - p)))   ＝⟨ ℚ+-assoc (f v) (1/4 * (q - p)) ((- f u) + (1/4 * (q - p))) ⁻¹ ⟩
            f v + 1/4 * (q - p) + ((- f u) + 1/4 * (q - p))     ＝⟨ ap (λ z → f v + 1/4 * (q - p) + ((- f u) + z)) (ℚ-minus-minus (1/4 * (q - p))) ⟩
            f v + 1/4 * (q - p) + ((- f u) - (- 1/4 * (q - p))) ＝⟨ ap (f v + 1/4 * (q - p) +_) (ℚ-minus-dist (f u) (- 1/4 * (q - p))) ⟩
            f v + 1/4 * (q - p) - (f u - 1/4 * (q - p)) ∎
      iv : f v - f u + (1/4 * (q - p) + 1/4 * (q - p)) < 1/4 * (q - p) + (1/4 * (q - p) + 1/4 * (q - p))
      iv = ℚ<-addition-preserves-order (f v - f u) (1/4 * (q - p)) (1/4 * (q - p) + 1/4 * (q - p)) (pr₂ (ℚ-abs-<-unpack (f v - f u) (1/4 * (q - p)) (pr₂ (is-continuous (1/4 * (q - p) , 0<1/4q-p)) v u vu)))
      xi : 3/4 * (q - p) ＝ 1/4 * (q - p) + (1/4 * (q - p) + 1/4 * (q - p))
      xi = 3/4 * (q - p)                                   ＝⟨ ℚ-distributivity' (q - p) 1/4 1/2 ⟩
           1/4 * (q - p) + 1/2 * (q - p)                   ＝⟨ ap (1/4 * (q - p) +_) (ℚ-distributivity' (q - p) 1/4 1/4) ⟩
           1/4 * (q - p) + (1/4 * (q - p) + 1/4 * (q - p)) ∎
      xii : 3/4 * (q - p) < (q - p)
      xii = transport (3/4 * (q - p) <_)
               (ℚ-mult-left-id (q - p)) (
                (ℚ<-pos-multiplication-preserves-order' 3/4 1ℚ (q - p) (0 , refl) (ℚ<-difference-positive p q p<q)))
      ...
      IV l = inr ∣ (u , v , 1/4 * (q - p) , 0<1/4q-p) , u<x , x<v , uv , (ℚ<-subtraction-preserves-order''' (f v) (1/4 * (q - p)) q l) ∣
         ...
         III (inl l₅)       = ℚ<-not-itself (ε + ε') (ℚ<-trans (ε + ε') (abs (f u - f v')) (ε + ε') iv using-continuity)
          where
           ...
           i = ℚ<-trans (f v' + ε') p (f u - ε)
               (ℚ<-subtraction-preserves-order'' (f v') p ε' l₄)
                (ℚ<-subtraction-preserves-order''' p ε (f u) l₂)
           ii : ε + ε' < f u - f v'
           ii = transport₂ _<_ α β (ℚ<-addition-preserves-order (f v' + ε') (f u - ε) (ε - f v') i)
            where
             α : f v' + ε' + (ε - f v') ＝ ε + ε'
             α = f v' + ε' + (ε - f v')     ＝⟨ ℚ+-assoc (f v' + ε') ε (- f v') ⁻¹                    ⟩
                 f v' + ε' + ε - f v'       ＝⟨ ap (_- f v') (ℚ+-assoc (f v') ε' ε)                   ⟩
                 f v' + (ε' + ε) - f v'     ＝⟨ ap (_- f v') (ℚ+-comm (f v') (ε' + ε))                   ⟩
                 ε' + ε + f v' - f v'       ＝⟨ ℚ+-assoc (ε' + ε) (f v') (- f v')                     ⟩
                 ε' + ε + (f v' - f v')     ＝⟨ ap₂ _+_ (ℚ+-comm ε' ε) (ℚ-inverse-sum-to-zero (f v')) ⟩
                 ε + ε' + 0ℚ                ＝⟨ ℚ-zero-right-neutral (ε + ε')                         ⟩
                 ε + ε'                     ∎
             β : f u - ε + (ε - f v') ＝ f u - f v'
             β = f u - ε + (ε - f v')     ＝⟨ ℚ+-assoc (f u - ε) ε (- f v') ⁻¹                     ⟩
                 f u - ε + ε - f v'       ＝⟨ ap (_- f v') (ℚ+-assoc (f u) (- ε) ε)                ⟩
                 f u + ((- ε) + ε) - f v' ＝⟨ ap (λ z → f u + z - f v') (ℚ-inverse-sum-to-zero' ε) ⟩
                 f u + 0ℚ - f v'          ＝⟨ ap (_- f v') (ℚ-zero-right-neutral (f u))            ⟩
                 f u - f v'               ∎
           iv : ε + ε' < abs (f u - f v')
           iv = ℚ<-≤-trans (ε + ε') (f u - f v') (abs (f u - f v')) ii (pr₂ (ℚ-abs-≤  (f u - f v')))
         III (inr l₅)  = ℚ<-not-itself (δ₁ + δ₂) (ℚ≤-<-trans (δ₁ + δ₂) (abs (u - v')) (δ₁ + δ₂) iv ii)
          where
           ...
           iv : δ₁ + δ₂ ≤ abs (u - v')
           iv = ℚ≤-trans (δ₁ + δ₂) δ (abs (u - v')) iii l₆
